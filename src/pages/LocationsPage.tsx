...
export function LocationsPage() {
  ...
  return (
    <div style={page}>
      <header style={header}>
        <div>
          <p style={eyebrow}>NETWORK SETUP</p>
          <h1 style={title}>Locations</h1>
          <p style={sub}>Manage branches, audio zones and player pairing.</p>
        </div>
        {["owner", "operations"].includes(workspace?.role ?? "") && (
          <button onClick={() => nav("/locations/new")} style={primary}>
      ...
      {loading ? (
        ...
        <div style={empty}>
          <Building2 size={28} color="var(--signal)" />
          <h2>No locations yet</h2>
          <p>Add your first branch to create audio zones and pair a player.</p>
          <button onClick={() => nav("/locations/new")} style={primary}>
            Add first location
          </button>
        </div>
        ...
        >
          {items.map((l) => {
            const zones = l.audio_zones ?? []
            const players = zones.flatMap((z) => z.players ?? [])
            ...
            return (
              ...
              >
                ...
                >
                  <div>
                    <h2 style={{ margin: 0, fontSize: 17 }}>{l.name}</h2>
                    <p
                      style={{
                        margin: "5px 0 0",
                        fontSize: 13,
                    ...
                    >
                      {zones.length} audio zone{zones.length !== 1 ? "s" : ""}
                    </p>
                  </div>
                  <ChevronRight color="var(--signal)" />
                </div>
...
export function AddLocationPage() {
  ...
  if (!["owner", "operations"].includes(workspace?.role ?? ""))
    return (
      <div style={page}>
        <h1 style={title}>Access denied</h1>
        <p style={sub}>
          Only owners and operations managers can add a location.
        </p>
      </div>
    )
  const save = async (e: FormEvent) => {
    ...
    const { data: z, error: ze } = await supabase
      .from("audio_zones")
      .insert({ location_id: loc.id, name: zone })
      .select()
      .single()
    if (ze) {
      setError(ze.message)
    ...
    await supabase
      ...
      .insert({
        zone_id: z.id,
        device_code: `BC-${crypto.randomUUID().slice(0, 8).toUpperCase()}`,
        display_name: `${name} player`,
        state: "unpaired",
        pairing_code: crypto.randomUUID().slice(0, 6).toUpperCase(),
        pairing_expires_at: new Date(Date.now() + 15 * 60 * 1000).toISOString(),
      })
    nav(`/locations/${loc.id}`)
  }
  return (
    <div style={page}>
      <Link
        to="/locations"
        style={{ color: "var(--signal)", textDecoration: "none", fontSize: 13 }}
      >
        ← Locations
      </Link>
      <form
      ...
      >
        <p style={eyebrow}>NEW BRANCH</p>
        <h1 style={title}>Add a location</h1>
        <label style={label}>
          Location name
          <input
            style={field}
        ...
        <button style={primary} disabled={busy}>
          {busy ? "Saving…" : "Create location & pairing code"}
        </button>
      </form>
    </div>
  )
...
function LocationDetail() {
  ...
  const invokeManager = async (
    ...
    if (!token)
      return {
        data: null,
        error: new Error("Manager session expired. Sign in again."),
      }
    return supabase.functions.invoke("player-manager-control", {
  ...
  const regenerate = async (playerId: string) => {
    ...
    setError("")
    setMessage("")
    const result = await invokeManager(playerId, "regenerate")
    if (result.error) setError(result.error.message)
    else {
      setMessage(
        "New code generated. The active browser session remains online.",
      )
      await load()
    }
  ...
  const stop = async (playerId: string) => {
    ...
    setError("")
    setMessage("")
    const result = await invokeManager(playerId, "stop")
    if (result.error) setError(result.error.message)
    else {
      setMessage(
        "Playback stopped and a new code was generated. The browser remains online.",
      )
      await load()
    }
    setStopping(null)
  ...
  return (
    <div style={page}>
      ...
      <header style={{ ...header, marginTop: 16 }}>
        <div>
          <p style={eyebrow}>LOCATION</p>
          <h1 style={title}>{location.name}</h1>
          <p style={sub}>Audio zones and player pairing status.</p>
        </div>
      </header>
            ...
            >
              <div>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <MapPin size={16} color="var(--signal)" />
                  <h2 style={{ fontSize: 16, margin: 0 }}>{z.name}</h2>
                </div>
                <p
                  style={{
                    margin: "8px 0 0",
                ...
                >
                  ...
                  {p.pairing_code && (
                    ...
                    >
                      <button
                        type="button"
                        onClick={() => void copyCode(p.pairing_code!)}
                        style={linkButton}
                      >
                      ...
                      <a
                        ...
                        style={linkButton}
                      >
                        <ExternalLink size={13} /> Open Player Mode
                      </a>
                    </div>
                  )}
