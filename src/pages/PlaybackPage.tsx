...
type ContentItem = { id: string; title: string; kind: string; state: string };
...
export function PlaybackPage() {
  ...
  useEffect(() => { const load = async () => { if (!db || !workspace) return; setLoading(true); const [playersResult, contentResult] = await Promise.all([db.from('players').select('id,display_name,state,last_seen_at').order('display_name'), db.from('content_items').select('id,title,kind,state').eq('organization_id', workspace.id).eq('state', 'ready').order('title')]); const next = (playersResult.data ?? []) as Player[]; const assets = (contentResult.data ?? []) as ContentItem[]; setPlayers(next); setContent(assets); setSelected((current) => current || next[0]?.id || ''); setSelectedContent((current) => current || assets[0]?.id || ''); setError(playersResult.error?.message ?? contentResult.error?.message ?? ''); setLoading(false); }; void load(); }, [workspace?.id]);
  const player = players.find((item) => item.id === selected);
  const asset = content.find((item) => item.id === selectedContent);
  const online = player?.state === 'online';
  const sendCommand = async (command: 'play' | 'pause' | 'skip') => { if (!player || !online) return; if (command === 'play' && !asset) { setError('Select an audio asset before pressing Play.'); return; } setMessage(''); setError(''); const result = await db.from('player_commands').insert({ player_id: player.id, command, payload: command === 'play' ? { content_item_id: asset?.id } : {}, status: 'pending' }); if (result.error) setError(result.error.message); else setMessage(`${command[0].toUpperCase()}${command.slice(1)} command sent.`); };
  return <div style={page}><p style={eyebrow}>PLAYBACK RUNTIME</p><h1 style={title}>Now playing</h1><p style={sub}>Control playback for a connected browser player.</p><section style={card}><label style={label}>Player<select style={select} value={selected} onChange={(event) => setSelected(event.target.value)}><option value="">Select a player</option>{players.map((item) => <option value={item.id} key={item.id}>{item.display_name || 'Unnamed player'}</option>)}</select></label><label style={{...label,marginTop:16}}>Audio asset<select style={select} value={selectedContent} onChange={(event) => setSelectedContent(event.target.value)}><option value="">Select an audio asset</option>{content.map((item) => <option value={item.id} key={item.id}>{item.title}</option>)}</select></label>{loading ? <p style={sub}>Loading player runtime…</p> : !player ? <div style={empty}><Radio size={24} /><strong>No player selected</strong><span>Open /player in a desktop or mobile browser and enter the pairing code first.</span></div> : <><div style={now}><span style={art}><Radio size={24} /></span><div><small>NOW PLAYING</small><h2>{online ? 'Browser player connected' : 'Waiting for browser player'}</h2><p>{online ? (asset ? `Ready to play ${asset.title}.` : 'Select an audio asset to start playback.') : 'Open /player on the device browser, enter its pairing code, and keep the tab open.'}</p></div><span style={status(online)}>{online ? 'Online' : 'Offline'}</span></div>{message && <p style={success}>{message}</p>}{error && <p style={warning}>{error}</p>}<div style={controls}><button disabled={!online || !asset} style={control} onClick={() => void sendCommand('play')}><Play size={17} /> Play</button><button disabled={!online} style={control} onClick={() => void sendCommand('pause')}><Pause size={17} /> Pause</button><button disabled={!online} style={control} onClick={() => void sendCommand('skip')}><SkipForward size={17} /> Skip</button><span style={volume}><Volume2 size={17} /> Volume 70%</span></div>{!online && <p style={warning}><AlertTriangle size={15} /> Connect the browser player to enable remote playback commands.</p>}</>}</section></div>;
}
