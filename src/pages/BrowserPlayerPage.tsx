...
export function BrowserPlayerPage() {
  ...
  const pair = async (event: FormEvent) => {
    ...
    setMessage("")
    try {
      const data = await call("player-pair", {
        pairing_code: code.trim().toUpperCase(),
        agent_version: "browser/1.0",
      })
      if (!data.browser_session_token || !data.player)
        throw new Error("Pairing response is incomplete.")
      const next = {
        token: data.browser_session_token,
        player: data.player as PlayerIdentity,
      }
      localStorage.setItem(STORAGE_KEY, JSON.stringify(next))
      setSession(next)
      setCode("")
      setMessage("Player connected. This browser is now the player.")
    } catch (caught) {
  ...
  const heartbeat = useCallback(async () => {
    if (!session) return
    try {
      await call("player-browser-heartbeat", {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const data = await call("player-browser-runtime", {
          session_token: session.token,
          action: "poll",
        })
        const command = data.command as {
          id: string
          command: string
          payload?: { audio_url?: string title?: string }
        } | null
        if (command) {
          try {
            if (command.command === "play") {
              ...
              if (!url)
                throw new Error(
                  "The selected audio asset has no playable file.",
                )
              if (audioRef.src !== url) {
                audioRef.src = url
                audioRef.load()
              ...
              try {
                ...
                throw new Error(
                  "Browser audio is blocked. Tap Enable audio, then press Play again.",
                )
              }
              ...
              setTrack("No audio selected")
            }
            setMessage(`Command completed: ${command.command}`)
            await call("player-browser-runtime", {
              session_token: session.token,
              action: "ack",
  ...
  const disconnect = async () => {
    localStorage.removeItem(STORAGE_KEY)
    setSession(null)
    setMessage("Browser player disconnected.")
    setError("")
  }
  const statusText = useMemo(
    () =>
      lastHeartbeat
        ? `Connected · last heartbeat ${lastHeartbeat}`
        : "Connecting…",
    [lastHeartbeat],
  )
  ...
  if (!session)
    return (
      <main style={shell}>
        <section style={card}>
          <div style={brand}>
            Branch<span>Cast</span>
          </div>
          <div style={icon}>
            <Radio size={30} />
          </div>
          <p style={eyebrow}>BROWSER PLAYER</p>
          <h1 style={title}>Connect this browser</h1>
          <p style={sub}>
            Open this page on the desktop or mobile browser that should play
            this branch’s audio. Enter the pairing code from the manager’s
            Location page.
          </p>
          <form onSubmit={pair} style={form}>
            <label style={label}>
              Pairing code
              <input
                autoFocus
                ...
                placeholder="e.g. 7E35ED"
                style={input}
              />
            </label>
            {error && <p style={errorText}>{error}</p>}
            <button disabled={busy || !code.trim()} style={primary}>
              {busy ? "Connecting…" : "Connect browser player"}
            </button>
          </form>
          <p style={hint}>
            This page does not require a manager login. Keep this tab open while
  ...
  return (
    <main style={shell}>
      <section style={card}>
        ...
        <p style={eyebrow}>BROWSER PLAYER</p>
        <h1 style={title}>
          {session.player.display_name || "BranchCast player"}
        </h1>
        <p style={sub}>
          This browser is connected and sending its heartbeat to BranchCast.
        </p>
        <div style={connected}>
        ...
        <div style={nowPlaying}>
          <span>{playing ? <Play size={17} /> : <Pause size={17} />}</span>
          <div>
            <strong>{track}</strong>
            <small>{playing ? "Playing in this browser" : "Paused"}</small>
          </div>
        </div>
        {!audioReady && (
          <button
        ...
        <div style={actions}>
          <button
            ...
            style={secondary}
          >
            <RefreshCw size={15} /> Send heartbeat
          </button>
          <button
            type="button"
            onClick={() => {
        ...
        <p style={hint}>
          Leave this tab open. The manager can now use Playback for this player.
        </p>
      </section>
    </main>
  )
