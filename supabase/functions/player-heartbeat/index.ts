...
Deno.serve(async (request) => {
  ...
  let body: { device_code?: string; state?: string; firmware_version?: string; metadata?: Record<string, unknown> }
  ...
  const update = await admin.from('players').update({ state: body.state === 'offline' ? 'offline' : 'online', firmware_version: body.firmware_version ?? null, last_seen_at: now }).eq('device_code', body.device_code).select('id,display_name,state,last_seen_at').maybeSingle()
