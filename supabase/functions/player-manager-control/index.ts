...
const fail = (error: string) =>
  new Response(JSON.stringify({ ok: false, error }), { status: 200, headers })
...
Deno.serve(async (request) => {
  ...
  if (!body.player_id || !body.action)
    return fail("player_id and action are required")
  ...
  if (identity.error || !identity.data.user)
    return fail("Manager session is invalid")
  ...
  if (zone.error || !zone.data)
    return fail(zone.error?.message || "Player zone not found")
  ...
  if (location.error || !location.data)
    return fail(location.error?.message || "Player location not found")
